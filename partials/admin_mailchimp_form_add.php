...
<div class="wrap choobs-wp-mailchimp-options">
	<h2><?php _e('Add Subscription Forms', $this->plugin_slug); ?></h2>
	<?php
		settings_errors();
		if( !empty( $error ) ) {
	...
	?>
			<form method="post" class="form" action="admin.php?page=choobs-wp-mailchimp-subscription-forms&do=add">
					...
					<div class="form-control" id="available_fields">
						...
						if( !empty( $ids ) ) {
							...
							foreach ( $vars['data'] as $list_vars ) {
								?>
								<table id="list_<?php echo $list_vars['id']; ?>" class="hidden">
									<thead>
									<tr>
				...
				<div class="form-group">
					...
					<input type="text" name="form_label" id="form_label" class="form-control" value="<?php echo ( isset( $_POST['form_label'] ) ? $_POST['form_label'] : '' ); ?>" placeholder="<?php _e( 'Something to identify the form later', $this->plugin_slug ); ?>" required />
				</div>
				<div class="form-group">
					<label for="form_html"><?php _e( 'Form HTML', $this->plugin_slug ); ?></label>
					<textarea name="form_html" id="form_html" class="form-control" placeholder="<?php _e( 'HTML to display the form', $this->plugin_slug ); ?>" required><?php echo ( isset( $_POST['form_label'] ) ? $_POST['form_html'] : '&lt;input type="email" name="EMAIL" value="" /&gt;' ); ?></textarea><br />
				...
				<div class="form-group">
					...
					<select name="form_status" id="form_status" class="form-control">
						<option value="draft"><?php _e( 'Draft', $this->plugin_slug ); ?></option>
						<option value="inactive"><?php _e( 'Inactive', $this->plugin_slug ); ?></option>
						<option value="active"><?php _e( 'Active', $this->plugin_slug ); ?></option>
					</select>
				</div>
